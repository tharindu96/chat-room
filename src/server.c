...
void handle_listener(int sockfd, int epfd);
void handle_request(int sockfd, int epfd);
...
int main(int argc, char **argv)
{
	...
	int sockfd = server_tcp_listen(SERVER_PORT);
...
void handle_listener(int sockfd, int epfd)
{
	...
	while (new_conn != -1) {
		new_conn = server_accept(sockfd, NULL, NULL);
		if (new_conn == -1) break;
		ev.events = EPOLLIN | EPOLLPRI | EPOLLERR | EPOLLHUP;
		ev.data.fd = new_conn;
